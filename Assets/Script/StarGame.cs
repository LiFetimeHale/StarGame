...
public class StarGame : MonoBehaviour
{
    ...
    private void ShowDirectionBtn()
    {
        ...
        if (curLStarPos.y < gameData.row - 1 && gameMap[(int)curLStarPos.x, (int)curLStarPos.y + 1] != 1)
        {
        ...
        if (curLStarPos.y > 0 && gameMap[(int)curLStarPos.x, (int)curLStarPos.y - 1] != 1)
        {
        ...
        if (curLStarPos.x > 0 && gameMap[(int)curLStarPos.x - 1, (int)curLStarPos.y] != 1)
        {
        ...
        if (curLStarPos.x < gameData.col - 1 && gameMap[(int)curLStarPos.x + 1, (int)curLStarPos.y] != 1)
        {
    ...
    private void OnTweenPosFinished()
    {
        if (curLStarPos == gameData.bigStarPos)
        {
            ...
            if (MainData.Instance.IsPlayEffect)
                NGUITools.PlaySound(audioList[2], volume, pitch);
            ...
            || curLStarPos.y < 0 || curLStarPos.y > gameData.row - 1)
        {
            Debug.Log("失败");
            if (MainData.Instance.IsPlayEffect)
                NGUITools.PlaySound(audioList[3], volume, pitch);
        ...
        if (gameMap[(int)curLStarPos.x, (int)curLStarPos.y] == 2)
        {
            for (int i = 0; i < gameData.transferList.Count; i++)
            {
                if (gameData.transferList[i].from.x == curLStarPos.x && gameData.transferList[i].from.y == curLStarPos.y)
                {
                    ...
                    if(MainData.Instance.IsPlayEffect)
                        NGUITools.PlaySound(audioList[1], volume, pitch);
    ...
    private void Transfer()
    {
        switch (dir)
        {
            case Direction.kUp:
                {
                    float tempMaxPosY = gameData.row;
            ...
            case Direction.kRight:
                {
                    float tempMaxPosX = gameData.col;

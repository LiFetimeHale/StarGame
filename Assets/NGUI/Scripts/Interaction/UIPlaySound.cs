...
public class UIPlaySound : MonoBehaviour
{
	...
	void OnPress (bool isPressed)
	{
		...
		if (canPlay && ((isPressed && trigger == Trigger.OnPress) || (!isPressed && trigger == Trigger.OnRelease)) && MainData.Instance.IsPlayEffect)
			NGUITools.PlaySound(audioClip, volume, pitch);
	}

	void OnClick ()
	{
        if (canPlay && trigger == Trigger.OnClick && MainData.Instance.IsPlayEffect)
			NGUITools.PlaySound(audioClip, volume, pitch);
	}
